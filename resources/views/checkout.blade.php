    ...
    <div class="content">
        <section class="contained">
            ...
            <div class="relative">
                ...
                <form class="checkoutform" method="POST" class="register" id="checkout-form">
                    {{ csrf_field() }}
                    ...
                    <table>
                        <tbody>
                            ...
                            <tr>
                                <td>CVV Code:</td>
                                <td>
                                    <input id="cvc" type="text" name="cvc" placeholder="CVV" pattern="[0-9]*"
                                        maxlength="4" required="required" onkeypress="return onlyNumberKey(event)" />
                            ...
                            <tr>
                                ...
                                <td>
                                    <input id="email" type="email" name="email" required="required"
                                        autocomplete="on" maxlength="40" placeholder="Email" value="{{ $user->email }}" />
                                </td>
                            </tr>
                            <?php
                            $fname = '';
                                ...
                                <td>
                                    <select name="country" id="country" class="input" style="max-width:165px;" required>
                                        ...
                                        @foreach ($countries as $c)
                                            <option value="{{ $c->code }}"
                                                {{ $user->country == $c->code ? 'selected' : '' }}>{{ $c->name }}</option>
                                        @endforeach
                                    </select>
                                </td>
                            </tr>
                            ...
                            <tr>
                                <td>
                                    ...
                                    <input type="hidden" id="fileids" name="fileids" value="{{ $audioids }}"
                                        id="paypal_audio_ids" />
                                    <input type="hidden" name="totalduration" value=""
                                        id="paypal_total_duration" />
                                    <!-- file data pass -->
                                    <button id="input-button" name="submit" type="button"
                                        onclick="window.location='{{ URL::to('/') }}/account'"
                                        class="c-btn">Cancel</button>
                                </td>
                                <td><button id="process-button" name="submit" type="button" onclick="checkout();"
                                        class="c-btn">Process Payment
                                    </button></td>
                            </tr>
                        </tbody>
                    </table>
