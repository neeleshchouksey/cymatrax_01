...
<section class="contained">
    ...
    <div class="checkouttotal">
        ...
        @if($remaining_file_limits ==0 || $remaining_file_limits < 0) <button id="clean-btn" class="c-btn"
            onclick="clean_files({{$id. ','.$remaining_file_limits}})" style="margin-top: 1rem; margin-bottom: 20px;">
            Clean File(s)
            </button>

            <div id="file_count" class="row">
                <span> Plan Limit is exceeded </span>
            </div>
            @else
            ...
            @elseif(Auth::user()->trial_expiry_date<time()) <button class="c-btn"
                onclick="document.location = '{{URL::to('/')}}/checkout/{{$id}}'"
                style="margin-top: 1rem; margin-bottom: 20px;">Proceed to Checkout
                </button>
                @else
                <button id="clean-btn" class="c-btn" onclick="clean_files({{$id. ','.$remaining_file_limits}})"
